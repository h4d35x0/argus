#!/usr/bin/env python3
"""
Generate a minimal LVGL 9.x bitmap font from a TTF.

Emits a .c file with the same structure lv_font_conv would produce, but
covering only the characters the digital clock actually renders. Keeps
flash cost down to ~10-15 KB at 96 px while still letting us call it via
the existing `lv_font_t *` font slot.

Tuned for the t-watch-ultra-clock project; uses Montserrat-Medium.ttf
shipped with the LVGL library, 4 bits per pixel, no compression.
"""

import sys
import freetype

# ---- inputs (argv-driven so the same generator emits every clock size) ------
#   python gen_clock_font.py <ttf> <size_px> <out.c> <font_var>
# Defaults keep the original Montserrat-96 behavior when run with no args.

import os
_defttf = "/home/adobeflash/Downloads/twatch-ultra-clock/.pio/libdeps/twatch_ultra/lvgl/scripts/built_in_font/Montserrat-Medium.ttf"
TTF_PATH = sys.argv[1] if len(sys.argv) > 1 else _defttf
SIZE_PX  = int(sys.argv[2]) if len(sys.argv) > 2 else 96
BPP      = 4
OUT_PATH = sys.argv[3] if len(sys.argv) > 3 else "src/lv_font_montserrat_clock_96.c"
FONT_VAR = sys.argv[4] if len(sys.argv) > 4 else "lv_font_montserrat_clock_96"
FONT_NAME = os.path.splitext(os.path.basename(TTF_PATH))[0]   # for the file header

# Characters to include. Default = the clock subset (digits, colon, space, A/M/P).
# Pass "ASCII" as argv[5] for the full printable-ASCII range (UI labels/readouts),
# or an explicit literal string of glyphs.
_defchars = " 0123456789:AMP"
_arg = sys.argv[5] if len(sys.argv) > 5 else _defchars
if _arg == "ASCII":
    CHARS = "".join(chr(c) for c in range(0x20, 0x7F))
else:
    CHARS = _arg

# ---- fallback font (argv[6]) -----------------------------------------------
#
# THIS MUST NOT DEFAULT TO NOTHING, and that is the whole point of it being
# here. Every font this script emits is an ASCII-ONLY SUBSET
# (U+0020..U+007E at most). An LV_SYMBOL_* glyph is a FontAwesome codepoint in
# the U+F000 block, so on a label styled with one of these faces it resolves to
# no glyph and LVGL draws NOTHING - silently, with no compile-time or runtime
# error. That shipped for months: the Threat Radar banner rendered
# "SOMEONE IS FOLLOWING YOU" with a broken glyph, and an earlier audit "closed"
# the bug by checking only the mono faces.
#
# So a generated font gets a fallback by DEFAULT, chosen as the nearest built-in
# montserrat. Every montserrat size 14..48 is already referenced elsewhere in
# src/, so the pointer costs no additional flash - it only names font data that
# is already linked.
#
# Pass "NONE" explicitly to opt out. Do that only for a face where a missing
# glyph must stay visible as a tofu box, and say why at the call site.
_MONTSERRAT_SIZES = (14, 16, 20, 24, 28, 32, 36, 48)

def _nearest_montserrat(px):
    return min(_MONTSERRAT_SIZES, key=lambda s: (abs(s - px), s))

_fb_arg = sys.argv[6] if len(sys.argv) > 6 else ""
if _fb_arg == "NONE":
    FALLBACK = None
elif _fb_arg:
    FALLBACK = _fb_arg
else:
    FALLBACK = "lv_font_montserrat_%d" % _nearest_montserrat(SIZE_PX)

# ---- helpers ---------------------------------------------------------------

face = freetype.Face(TTF_PATH)
face.set_pixel_sizes(0, SIZE_PX)

# Render each glyph + collect metrics.
glyphs = []     # list of dicts {codepoint, adv_w, box_w, box_h, ofs_x, ofs_y, bitmap (bytes)}
for ch in CHARS:
    codepoint = ord(ch)
    face.load_char(ch, freetype.FT_LOAD_RENDER | freetype.FT_LOAD_TARGET_NORMAL)
    g = face.glyph
    bm = g.bitmap
    # FreeType returns metrics in 26.6 fixed point. LVGL stores adv_w in 1/16 px,
    # so divide by 4 (=64/16) to convert.
    adv_w = (g.metrics.horiAdvance + 2) // 4

    # Convert grayscale 8bpp → 4bpp (15 levels). Use perceptually flat
    # quantization: (v * 15 + 127) / 255, the same rounding lv_font_conv uses.
    w, h = bm.width, bm.rows
    pitch = bm.pitch
    pixels = []
    for y in range(h):
        row = bm.buffer[y*pitch:(y+1)*pitch]
        for x in range(w):
            v8 = row[x]
            v4 = (v8 * 15 + 127) // 255
            pixels.append(v4 & 0x0F)

    # Pack nibbles: pixel 0 → high nibble of byte 0, pixel 1 → low, etc.
    packed = bytearray()
    for i in range(0, len(pixels), 2):
        hi = pixels[i] << 4
        lo = pixels[i+1] if i + 1 < len(pixels) else 0
        packed.append(hi | lo)

    glyphs.append({
        "ch":         ch,
        "codepoint":  codepoint,
        "adv_w":      int(adv_w),
        "box_w":      int(w),
        "box_h":      int(h),
        "ofs_x":      int(g.bitmap_left),
        # LVGL's ofs_y is the offset from the baseline to the bottom of
        # the bitmap. FreeType's bitmap_top is from the baseline to the
        # top of the bitmap. Convert: ofs_y = bitmap_top - box_h.
        "ofs_y":      int(g.bitmap_top - h),
        "bitmap":     bytes(packed),
    })

# Stable insertion order by codepoint so cmaps are simple.
glyphs.sort(key=lambda g: g["codepoint"])

# Compute bitmap_index for each glyph (running byte offset).
bitmap_index = 0
for g in glyphs:
    g["bitmap_index"] = bitmap_index
    bitmap_index += len(g["bitmap"])

# Font-wide metrics: line height and ascender/descender from face metrics
# (also 26.6 FP, also converted to pixels).
size_metrics = face.size
line_height  = (size_metrics.height + 32) >> 6
ascender     = (size_metrics.ascender + 32) >> 6
descender    = (-size_metrics.descender + 32) >> 6
# LVGL's base_line is descent (pixels below baseline that still belong to glyphs).
base_line    = descender

# ---- emit C ---------------------------------------------------------------

def fmt_bytes_per_line(data, n=12, indent="    "):
    out = []
    for i in range(0, len(data), n):
        chunk = data[i:i+n]
        out.append(indent + ", ".join(f"0x{b:02x}" for b in chunk) + ",")
    return "\n".join(out)

# Glyph 0 is reserved (placeholder for "not found"); real glyphs start at id=1.
# A sparse cmap is the simplest correct path for our scattered set: one
# big unicode_list with per-glyph offsets, glyph_id_start=1.
unicode_list = [g["codepoint"] for g in glyphs]
glyph_id_list = list(range(1, len(glyphs) + 1))

# Rendered into the public struct below. Emitted as an explicit NULL when the
# caller opted out, so the field is never silently absent - an absent field and
# a deliberate NULL read very differently to the next person.
if FALLBACK:
    fallback_line = (
        "    // ASCII-only subset: fall back to montserrat so an LV_SYMBOL_*\n"
        "    // glyph renders instead of silently drawing nothing. montserrat is\n"
        "    // already linked elsewhere, so this costs no flash.\n"
        "    .fallback = &%s,\n" % FALLBACK)
else:
    fallback_line = "    .fallback = NULL,   /* opted out via argv[6]=NONE */\n"

with open(OUT_PATH, "w") as f:
    f.write(f"""/*
 * Custom {FONT_NAME} {SIZE_PX} px subset font for the home-screen digital
 * clock. Generated by tools/gen_clock_font.py from {FONT_NAME}.ttf.
 * (Variable name kept as {FONT_VAR} so it stays a drop-in for the clock slot.)
 *
 * Only includes the glyphs the clock actually uses (digits, colon,
 * space, A/M/P) so the flash cost stays tiny (~{(bitmap_index/1024):.1f} KB
 * of glyph bitmap data) compared with a full-character font at this size.
 *
 * The format mirrors what `lv_font_conv` produces: same struct layout,
 * cmap type, and glyph descriptor field order. This file plugs in via
 * the existing `lv_font_t *` slot exactly like the built-in fonts.
 */

#include "lvgl.h"

/*-----------------
 *    BITMAPS
 *----------------*/

static LV_ATTRIBUTE_LARGE_CONST const uint8_t glyph_bitmap[] = {{
""")
    for g in glyphs:
        f.write(f"    /* U+{g['codepoint']:04X} \"{g['ch']}\" */\n")
        if g["bitmap"]:
            f.write(fmt_bytes_per_line(g["bitmap"]) + "\n")
        f.write("\n")
    f.write("};\n\n")

    f.write("""/*---------------------
 *  GLYPH DESCRIPTION
 *--------------------*/

static const lv_font_fmt_txt_glyph_dsc_t glyph_dsc[] = {
    {.bitmap_index = 0, .adv_w = 0, .box_w = 0, .box_h = 0, .ofs_x = 0, .ofs_y = 0} /* id = 0 reserved */,
""")
    for g in glyphs:
        f.write(
            f"    {{.bitmap_index = {g['bitmap_index']}, "
            f".adv_w = {g['adv_w']}, "
            f".box_w = {g['box_w']}, "
            f".box_h = {g['box_h']}, "
            f".ofs_x = {g['ofs_x']}, "
            f".ofs_y = {g['ofs_y']}}},\n"
        )
    f.write("};\n\n")

    f.write("/*---------------------\n *  CHARACTER MAPPING\n *--------------------*/\n\n")
    f.write("static const uint16_t unicode_list[] = {\n    ")
    f.write(", ".join(f"0x{cp - unicode_list[0]:04X}" for cp in unicode_list))
    f.write("\n};\n\n")
    f.write("""static const lv_font_fmt_txt_cmap_t cmaps[] = {
    {
""")
    f.write(f"        .range_start = {unicode_list[0]}, ")
    f.write(f".range_length = {unicode_list[-1] - unicode_list[0] + 1}, ")
    f.write(f".glyph_id_start = 1,\n")
    f.write(f"        .unicode_list = unicode_list, .glyph_id_ofs_list = NULL, ")
    f.write(f".list_length = {len(unicode_list)}, ")
    f.write(f".type = LV_FONT_FMT_TXT_CMAP_SPARSE_TINY\n    }}\n}};\n\n")

    f.write(f"""/*--------------------
 *  ALL CUSTOM DATA
 *--------------------*/

static const lv_font_fmt_txt_dsc_t font_dsc = {{
    .glyph_bitmap = glyph_bitmap,
    .glyph_dsc = glyph_dsc,
    .cmaps = cmaps,
    .kern_dsc = NULL,
    .kern_scale = 0,
    .cmap_num = 1,
    .bpp = {BPP},
    .kern_classes = 0,
    .bitmap_format = 0,
}};

/*-----------------
 *  PUBLIC FONT
 *----------------*/

const lv_font_t {FONT_VAR} = {{
    .get_glyph_dsc    = lv_font_get_glyph_dsc_fmt_txt,
    .get_glyph_bitmap = lv_font_get_bitmap_fmt_txt,
    .line_height = {line_height},
    .base_line = {base_line},
    .subpx = LV_FONT_SUBPX_NONE,
    .underline_position = -8,
    .underline_thickness = 4,
    .dsc = &font_dsc,
{fallback_line}}};
""")

print(f"wrote {OUT_PATH}")
print(f"  {len(glyphs)} glyphs, bitmap = {bitmap_index} bytes")
print(f"  line_height = {line_height}, base_line = {base_line}, adv_w '0' = ", end="")
for g in glyphs:
    if g['ch'] == '0':
        print(f"{g['adv_w']/16:.2f} px")
        break
