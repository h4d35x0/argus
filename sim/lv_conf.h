/* SIM-ONLY lv_conf.h. Copied from lib/LilyGoLib/src/lv_conf.h and flipped in
 * exactly three places: SDL display on, CLIB malloc (the firmware supplies a
 * PSRAM allocator the host does not have), and logging on so a mis-set flag
 * says so instead of rendering a black window. Do NOT edit the device conf to
 * make the simulator build. */
/**
 * @file lv_conf.h
...
#if LV_USE_FS_STDIO
    #define LV_FS_STDIO_LETTER 'A'      /* same drive letter the icon loader uses */
    #define LV_FS_STDIO_PATH "sdroot"   /* host stand-in for the card root */
